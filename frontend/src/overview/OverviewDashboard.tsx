...
type FactoryProduct={month:string;factory:string;product_type:string;eff_pct:number};
type Payload={kpis:Partial<Record<Period,number>>;kpi_details:Partial<Record<Period,KpiDetail>>;factory_periods:PeriodRow[];monthly:{month:string;eff_pct:number}[];last30:{produce_date:string;eff_pct:number}[];last30_factory:{produce_date:string;factory:string;eff_pct:number}[];factory_monthly:{month:string;factory:string;eff_pct:number}[];factory_monthly_product:FactoryProduct[];latest_date:string|null;last_refresh:string};
...
const productColor=(name:string)=>{let h=0;for(let i=0;i<name.length;i++)h=((h<<5)-h+name.charCodeAt(i))|0;return PRODUCT_COLORS[Math.abs(h)%PRODUCT_COLORS.length]};
...
export default function OverviewDashboard(){
  ...
  const[startDate,setStartDate]=useState("");const[endDate,setEndDate]=useState("");const[filtersReady,setFiltersReady]=useState(false);const[factory,setFactory]=useState("ALL");
  ...
  useEffect(()=>{getJSON<Filters>("/api/overview/filters").then(nextFilters=>{setFilters(nextFilters);const defaultEnd=nextFilters.max_date||new Date().toISOString().slice(0,10);setEndDate(defaultEnd);setStartDate(yearStartFor(defaultEnd,nextFilters.min_date));setFiltersReady(true)}).catch(e=>setError(`Unable to load Overview filters: ${e.message}`))},[]);
  useEffect(()=>{if(!filtersReady||!effectiveStart||!effectiveEnd)return;let active=true;setLoading(true);setError("");const p:Record<string,string>={start_date:effectiveStart,end_date:effectiveEnd};if(effectiveFactory!=="ALL")p.factory=effectiveFactory;getJSON<Payload>("/api/overview/dashboard",p).then(r=>active&&setData(r)).catch(e=>active&&setError(`Unable to load Overview: ${e.message}`)).finally(()=>active&&setLoading(false));return()=>{active=false}},[filtersReady,effectiveStart,effectiveEnd,effectiveFactory]);
  ...
  const lineOption=(rows:{x:string;y:number}[],kind:"month"|"day")=>{const vals=rows.map(x=>Number(x.y)).filter(Number.isFinite);const avg=vals.length?vals.reduce((a,b)=>a+b,0)/vals.length:0;return{grid:{left:48,right:24,top:42,bottom:44},tooltip:{trigger:"axis",valueFormatter:(v:number)=>pct(v)},xAxis:{type:"category",boundaryGap:false,data:rows.map(x=>x.x),axisLabel:{fontSize:10,formatter:(v:string)=>kind==="month"?monthLabel(v):dayLabel(v)}},yAxis:{type:"value",axisLabel:{formatter:(v:number)=>`${Math.round(v*100)}%`},splitLine:{show:false}},series:[{type:"line",smooth:.22,symbol:"circle",symbolSize:6,lineStyle:{width:2.5,color:"#2864d7"},itemStyle:{color:"#18b56a"},data:rows.map(x=>x.y),label:{show:true,position:"top",distance:8,color:"#26364f",fontSize:kind==="day"?8:9,fontWeight:700,formatter:(p:any)=>pct(p.value,0)},labelLayout:{hideOverlap:false},markLine:{silent:true,symbol:"none",lineStyle:{color:"#aaa",type:"dashed"},label:{show:false},data:[{yAxis:avg}]}}]}};
  ...
  const last30Option=useMemo(()=>{const rows=last30.map(x=>({x:x.produce_date,y:Number(x.eff_pct)}));const option=lineOption(rows,"day") as any;option.tooltip={trigger:"axis",confine:true,backgroundColor:"#fff",borderColor:"#aeb8c6",borderWidth:1,padding:0,extraCssText:"box-shadow:0 8px 24px rgba(20,35,60,.22);border-radius:3px;",formatter:(items:any[])=>{const item=items?.[0];if(!item)return"";const day=String(item.axisValue??rows[item.dataIndex]?.x??"");const factories=(data?.last30_factory||[]).filter(x=>String(x.produce_date)===day&&x.eff_pct!=null).sort((a,b)=>Number(b.eff_pct)-Number(a.eff_pct));const detail=factories.length?factories.map(r=>`<div style="display:flex;align-items:center;justify-content:space-between;gap:16px;margin-top:8px"><span style="display:flex;align-items:center;gap:7px"><i style="display:inline-block;width:9px;height:9px;border-radius:50%;background:${COLORS[r.factory]||"#94a3b8"}"></i><span style="font-size:11px;color:#283247">${esc(r.factory)}</span></span><b style="font-size:11px;color:#172033">${pct(r.eff_pct,1)}</b></div>`).join(""):`<div style="margin-top:10px;color:#748196;font-size:11px">No Factory data</div>`;return `<div style="padding:12px 14px;min-width:220px"><div style="font-size:12px;font-weight:700;color:#263145">EFF% by FACTORY</div><div style="font-size:10px;color:#748196;margin-top:3px">${esc(dayLabel(day))}</div>${detail}</div>`}};return option},[last30,data]);
  const ribbonOption=useMemo(()=>{const src=data?.factory_monthly||[];const products=data?.factory_monthly_product||[];const months=Array.from(new Set(src.map(x=>x.month))).sort();const facs=Array.from(new Set(src.map(x=>x.factory))).sort((a,b)=>ORDER.indexOf(a)-ORDER.indexOf(b));const rank=new Map<string,Map<string,{r:number;e:number}>>();months.forEach(m=>{const rs=src.filter(x=>x.month===m).sort((a,b)=>Number(b.eff_pct)-Number(a.eff_pct));rank.set(m,new Map(rs.map((x,i)=>[x.factory,{r:rs.length-i,e:Number(x.eff_pct)}])))});return{legend:{top:0,data:facs,icon:"circle",itemWidth:9,itemHeight:9},grid:{left:24,right:22,top:40,bottom:38},tooltip:{trigger:"item",confine:true,backgroundColor:"#fff",borderColor:"#aeb8c6",borderWidth:1,padding:0,extraCssText:"box-shadow:0 8px 24px rgba(20,35,60,.22);border-radius:3px;",formatter:(p:any)=>{if(!p.data)return"";const month=String(p.data.month??"");const fac=String(p.seriesName??"");const rows=products.filter(x=>String(x.month)===month&&x.factory===fac&&x.eff_pct!=null).sort((a,b)=>Number(b.eff_pct)-Number(a.eff_pct));const max=Math.max(...rows.map(x=>Number(x.eff_pct)||0),.01);const detail=rows.length?rows.map(r=>{const width=Math.max(5,(Number(r.eff_pct)/max)*205);return `<div style="margin-top:10px"><div style="font-size:10px;color:#283247;margin-bottom:4px">${esc(r.product_type)}</div><div style="display:flex;align-items:center;gap:9px"><span style="display:block;width:${width}px;max-width:205px;height:15px;border-radius:2px;background:${productColor(r.product_type)}"></span><b style="font-size:11px;color:#172033">${pct(r.eff_pct,1)}</b></div></div>`}).join(""):`<div style="margin-top:10px;color:#748196;font-size:11px">No Product Type data</div>`;return `<div style="padding:12px 14px;min-width:270px"><div style="font-size:12px;font-weight:700;color:#263145">EFF% by Product Type</div><div style="font-size:10px;color:#748196;margin-top:3px">${esc(fac)} · ${esc(monthLabel(month))}</div>${detail}</div>`}},xAxis:{type:"category",data:months,axisLabel:{formatter:(v:string)=>monthLabel(v),fontSize:9}},yAxis:{type:"value",min:.5,max:Math.max(1.5,facs.length+.5),show:false},series:facs.map(f=>({name:f,type:"line",smooth:.42,symbol:"roundRect",symbolSize:[58,18],lineStyle:{width:16,color:COLORS[f],opacity:crossFactory&&crossFactory!==f?.14:.72},itemStyle:{color:COLORS[f]},data:months.map(m=>{const x=rank.get(m)?.get(f);return x?{value:x.r,e:x.e,month:m,label:{show:true,position:"inside",formatter:`${(x.e*100).toFixed(1)}%`,fontSize:8,fontWeight:700}}:null})}))}},[data,crossFactory]);
