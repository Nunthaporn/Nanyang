...
export default function ModelLineDashboard() {
  ...
  const [startDate, setStartDate] = useState(""); const [endDate, setEndDate] = useState(""); const [filtersReady, setFiltersReady] = useState(false);
  const [factory, setFactory] = useState("ALL"); const [summary, setSummary] = useState<Summary | null>(null);
  const [modelEff, setModelEff] = useState<ModelEff[]>([]); const [product, setProduct] = useState<ProductEff[]>([]); const [latest, setLatest] = useState<LatestLine[]>([]); const [dateModel, setDateModel] = useState<DateModel[]>([]);
  const [selectedModel, setSelectedModel] = useState<string | null>(null); const [loading, setLoading] = useState(false); const [error, setError] = useState("");

  useEffect(() => {
    getJSON<Filters>("/api/model-line/filters").then((nextFilters) => {
      setFilters(nextFilters); const defaultEnd = nextFilters.max_date || new Date().toISOString().slice(0, 10); setEndDate(defaultEnd); setStartDate(yearStartFor(defaultEnd, nextFilters.min_date)); setFiltersReady(true);
    }).catch((e) => setError(`Unable to load filters: ${e.message}`));
  }, []);
  useEffect(() => {
    if (!filtersReady || !startDate || !endDate) return; let active = true; setLoading(true); setError("");
    Promise.all([
      getJSON<Summary>("/api/model-line/summary", startDate, endDate, factory), getJSON<ModelEff[]>("/api/model-line/eff-by-model-line", startDate, endDate, factory),
      getJSON<ProductEff[]>("/api/model-line/product-table", startDate, endDate, factory), getJSON<LatestLine[]>("/api/model-line/latest-by-line", startDate, endDate, factory), getJSON<DateModel[]>("/api/model-line/date-model-line", startDate, endDate, factory),
    ]).then(([s, m, p, l, d]) => { if (!active) return; setSummary(s); setModelEff(m); setProduct(p); setLatest(l); setDateModel(d); setSelectedModel((current) => (current && m.some((x) => x.model_line === current) ? current : null)); })
  ...
  const factories = useMemo(() => { const values = filters.factories.length ? filters.factories : FACTORY_ORDER; return [...values].sort((a, b) => { const ai = FACTORY_ORDER.indexOf(a), bi = FACTORY_ORDER.indexOf(b); return (ai < 0 ? 99 : ai) - (bi < 0 ? 99 : bi) || a.localeCompare(b); }); }, [filters.factories]);
  ...
  const chartKey = selectedModel ?? "ALL";

  const modelBarOption = useMemo(() => ({ animationDuration: 350, grid: { left: 45, right: 64, top: 44, bottom: 34 }, tooltip: { trigger: "axis", axisPointer: { type: "shadow" }, formatter: (items: any[]) => { const p = items?.[0]; const r = visibleModelEff[p?.dataIndex]; if (!r) return ""; return `Model Line: ${esc(r.model_line)}<br/>EFF%: ${pct(r.eff_pct, 1)}`; } }, xAxis: { type: "category", data: visibleModelEff.map((x) => x.model_line), axisTick: { show: false }, axisLine: { show: false } }, yAxis: { type: "value", min: 0, max: 1.15, axisLabel: { formatter: (v: number) => `${Math.round(v * 100)}%` }, splitLine: { lineStyle: { color: "#dce6f1", type: "dashed" } } }, series: [{ type: "bar", barMaxWidth: 48, data: visibleModelEff.map((x) => ({ value: Number(x.eff_pct), itemStyle: { color: Number(x.eff_pct) >= TARGET ? "#22b956" : "#ef4760", borderRadius: [5, 5, 0, 0] } })), label: { show: true, position: "top", formatter: (p: any) => `${(Number(p.value) * 100).toFixed(1)}%`, fontSize: 11 }, markLine: { silent: true, symbol: "none", lineStyle: { color: "#6686cf", type: "dashed", width: 2 }, label: { formatter: "Target 65%", position: "end", color: "#3561b6", fontWeight: 700, fontSize: 10 }, data: [{ yAxis: TARGET }] } }] }), [visibleModelEff]);

  const productPivot = useMemo(() => { const models = Array.from(new Set(visibleProduct.map((x) => x.model_line))).sort(); const rows = Array.from(new Set(visibleProduct.map((x) => x.pd_type))).sort().map((pd) => { const byModel: Record<string, number | null> = {}; const source = visibleProduct.filter((x) => x.pd_type === pd); models.forEach((m) => { const r = source.find((x) => x.model_line === m); byModel[m] = r ? Number(r.eff_pct) : null; }); const valid = source.map((x) => Number(x.eff_pct)).filter(Number.isFinite); return { pd, byModel, total: valid.length ? valid.reduce((a, b) => a + b, 0) / valid.length : null }; }); return { models, rows }; }, [visibleProduct]);

  const makeLatestOption = (rows: LatestLine[], compact = false) => ({ animationDuration: 350, grid: { left: 48, right: 64, top: 44, bottom: compact ? 52 : 62 }, tooltip: { trigger: "axis", confine: true, backgroundColor: "#fff", borderColor: "#aeb8c6", borderWidth: 1, padding: 0, axisPointer: { type: "shadow" }, formatter: (items: any[]) => { const i = items[0]; const r = rows[i.dataIndex]; if (!r) return ""; const products = (r.product_types ?? []).filter(x => x.eff_pct != null).sort((a, b) => Number(b.eff_pct) - Number(a.eff_pct)); const maximum = Math.max(...products.map(x => Number(x.eff_pct) || 0), .01); const detail = products.length ? products.map(x => { const width = Math.max(5, (Number(x.eff_pct) / maximum) * 205); return `<div style="margin-top:10px"><div style="font-size:10px">${esc(x.product_type)}</div><div style="display:flex;align-items:center;gap:9px"><span style="display:block;width:${width}px;max-width:205px;height:15px;background:${productColor(x.product_type)}"></span><b>${pct(x.eff_pct,1)}</b></div></div>`; }).join("") : `<div style="margin-top:10px">No Product Type data</div>`; return `<div style="padding:12px 14px;min-width:270px"><div style="font-weight:700">EFF% by Product Type</div><div style="font-size:10px">${esc(r.model_line)} · Line ${esc(String(r.line))} · ${esc(String(r.latest_date ?? ""))}</div>${detail}</div>`; } }, xAxis: { type: "category", data: rows.map((x) => `${x.line}\n${x.model_line}`), axisLabel: { interval: 0, fontSize: compact ? 10 : 9 }, axisTick: { show: false } }, yAxis: { type: "value", min: 0, axisLabel: { formatter: (v: number) => `${Math.round(v * 100)}%` } }, dataZoom: rows.length > 14 ? [{ type: "inside", startValue: 0, endValue: 13, zoomLock: true, zoomOnMouseWheel: false, moveOnMouseWheel: false, moveOnMouseMove: true }, { type: "slider", startValue: 0, endValue: 13, zoomLock: true, height: 12, bottom: 4, showDetail: false }] : [], series: [{ type: "bar", barMaxWidth: 42, data: rows.map((x) => ({ value: Number(x.eff_pct), itemStyle: { color: Number(x.eff_pct) >= TARGET ? "#22b956" : "#ef1760", borderRadius: [4,4,0,0] } })), label: { show: true, position: "top", formatter: (p: any) => `${(Number(p.value) * 100).toFixed(1)}%`, fontSize: 10 }, markLine: { silent: true, symbol: "none", lineStyle: { color: "#6686cf", type: "dashed" }, label: { formatter: "Target 65%" }, data: [{ yAxis: TARGET }] } }] });

  const ribbonOption = useMemo(() => { const dates = Array.from(new Set(visibleDateModel.map((x) => x.produce_date))).sort(); const visibleDates = dates.slice(Math.max(0, dates.length - 18)); const models = Array.from(new Set(visibleDateModel.map((x) => x.model_line))).sort(); const byDate = new Map<string, Map<string, DateModel>>(); visibleDates.forEach((d) => byDate.set(d, new Map(visibleDateModel.filter((x) => x.produce_date === d).map((x) => [x.model_line, x])))); const values = visibleDateModel.filter((x) => visibleDates.includes(x.produce_date)).map((x) => Number(x.eff_pct)).filter(Number.isFinite); const minY = values.length ? Math.max(0, Math.floor((Math.min(...values) - .05) * 20) / 20) : 0; const maxY = values.length ? Math.min(1.15, Math.ceil((Math.max(...values) + .05) * 20) / 20) : 1; return { animationDuration: 450, color: models.map((m) => MODEL_COLORS[m] || "#64748b"), legend: { top: 0, data: models, icon: "circle" }, grid: { left: 48, right: 28, top: 42, bottom: 36 }, tooltip: { trigger: "item", confine: true, formatter: (p: any) => { const day = String(p.name ?? ""); const model = String(p.seriesName ?? ""); const r = byDate.get(day)?.get(model); if (!r) return ""; const products = (r.product_types ?? []).filter(x => x.eff_pct != null).sort((a,b) => Number(b.eff_pct)-Number(a.eff_pct)); const maximum = Math.max(...products.map(x => Number(x.eff_pct)||0), .01); const detail = products.length ? products.map(x => { const width = Math.max(5,(Number(x.eff_pct)/maximum)*205); return `<div style="margin-top:10px"><div>${esc(x.product_type)}</div><div style="display:flex;gap:9px;align-items:center"><span style="display:block;width:${width}px;height:15px;background:${productColor(x.product_type)}"></span><b>${pct(x.eff_pct,1)}</b></div></div>`; }).join("") : "No Product Type data"; return `<div style="padding:12px 14px"><div style="font-weight:700">EFF% by Product Type</div><div>${esc(model)} · ${esc(day)}</div>${detail}</div>`; } }, xAxis: { type: "category", data: visibleDates, boundaryGap: false }, yAxis: { type: "value", min: minY, max: maxY, axisLabel: { formatter: (v:number) => `${Math.round(v*100)}%` } }, series: models.map((m) => ({ name:m, type:"line", smooth:.18, symbol:"circle", symbolSize:5, connectNulls:true, data: visibleDates.map((d,i) => { const row=byDate.get(d)?.get(m); const value=row?Number(row.eff_pct):undefined; return Number.isFinite(value) ? { value, label:{ show:selectedModel ? true : i%3===0 || i===visibleDates.length-1, formatter:pct(value,1), position:"top" } } : null; }) })) }; }, [visibleDateModel, selectedModel]);
