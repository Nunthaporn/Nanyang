...
export const customerOption=(d:Customer[],factoryRows:CustomerFactory[],target:number,selectedCustomer:string|null):EChartsOption=>({
  ...
  series:[{type:"bar",barWidth:20,data:d.map(x=>({name:x.customer,value:x.eff_pct,itemStyle:{color:(x.eff_pct??0)>=target?"#23df7a":"#df8396",borderRadius:[0,5,5,0],opacity:!selectedCustomer||x.customer===selectedCustomer?1:.2}})),label:{show:true,position:"right",formatter:(x:any)=>x.value==null?"":pct(Number(x.value)),fontWeight:"bold",fontSize:10,distance:8},markLine:{symbol:"none",silent:true,lineStyle:{color:"#7ba4ec",type:"dashed",width:2},label:{show:true,formatter:"Target 60%",position:"insideEndTop",rotate:0,offset:[0,-10],align:"center",verticalAlign:"bottom",color:"#2767c8",fontWeight:700,fontSize:10,padding:[3,6],backgroundColor:"#edf4ff",borderRadius:8},data:[{xAxis:target}]}}]
});
