...
# EASY LEAN uses teffdata."EasyLean Line" as the line dimension for every factory.
DISPLAY_LINE = """
...
BASE_FILTER = f"""
    ...
    AND (CAST(:factory AS text) IS NULL OR UPPER(BTRIM(e."FACTORY"::text)) = UPPER(CAST(:factory AS text)))
    AND (CAST(:selected_factory AS text) IS NULL OR UPPER(BTRIM(e."FACTORY"::text)) = UPPER(CAST(:selected_factory AS text)))
    AND (CAST(:selected_line AS text) IS NULL OR ({DISPLAY_LINE} = CAST(:selected_line AS text)))
    AND (CAST(:selected_line_factory AS text) IS NULL OR UPPER(BTRIM(e."FACTORY"::text)) = UPPER(CAST(:selected_line_factory AS text)))
"""
...
# Power BI equivalent:
# Eff Latest date = CALCULATE([EFF%], LASTDATE('Date'[Date]))
# EFF% = DIVIDE([SumOutmin], [SumInmin])
# Factory dimension = mt_factory.FACTORY
# Line dimension = teffdata."EasyLean Line"
LATEST_BY_LINE_SQL = text(f"""
WITH factory_dim AS (
    SELECT DISTINCT UPPER(BTRIM(m."FACTORY"::text)) AS factory
    FROM public.mt_factory m
    WHERE NULLIF(BTRIM(m."FACTORY"::text), '') IS NOT NULL
),
latest_date AS (
    ...
    WHERE e."Date"::date BETWEEN :start_date AND :end_date
      AND (CAST(:factory AS text) IS NULL OR fd.factory = UPPER(CAST(:factory AS text)))
      AND (CAST(:selected_factory AS text) IS NULL OR fd.factory = UPPER(CAST(:selected_factory AS text)))
      AND (CAST(:selected_line_factory AS text) IS NULL OR fd.factory = UPPER(CAST(:selected_line_factory AS text)))
),
latest_data AS (
    SELECT
        fd.factory,
    ...
    WHERE e."Date"::date = ld.latest_date
      AND {DISPLAY_LINE} IS NOT NULL
      AND (CAST(:factory AS text) IS NULL OR fd.factory = UPPER(CAST(:factory AS text)))
      AND (CAST(:selected_factory AS text) IS NULL OR fd.factory = UPPER(CAST(:selected_factory AS text)))
      AND (CAST(:selected_line AS text) IS NULL OR {DISPLAY_LINE} = CAST(:selected_line AS text))
      AND (CAST(:selected_line_factory AS text) IS NULL OR fd.factory = UPPER(CAST(:selected_line_factory AS text)))
...
product_type_eff AS (
    SELECT
        ...
        SUM(min_output_num) / NULLIF(SUM(min_input_num), 0) AS eff_pct
    FROM latest_data
    GROUP BY factory, line, product_type
),
product_json AS (
    SELECT
        factory,
...
SELECT MIN(e."Date"::date) AS min_date, MAX(e."Date"::date) AS max_date,
       ARRAY_AGG(DISTINCT UPPER(BTRIM(m."FACTORY"::text)) ORDER BY UPPER(BTRIM(m."FACTORY"::text))) FILTER (WHERE NULLIF(BTRIM(m."FACTORY"::text), '') IS NOT NULL) AS factories
FROM public.teffdata e
CROSS JOIN public.mt_factory m
""")
