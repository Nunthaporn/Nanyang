...
BASE_FILTER = f"""
    ...
    AND (
        ...
        OR ({DISPLAY_LINE} = CAST(:selected_line AS text))
    )
    AND (
...
latest AS (
    SELECT MAX("Date"::date) AS data_as_of FROM filtered
)
SELECT latest.data_as_of,
       SUM(filtered.min_output_num) FILTER (WHERE filtered.display_line IS NOT NULL)
         / NULLIF(SUM(filtered.min_input_num) FILTER (WHERE filtered.display_line IS NOT NULL), 0) AS eff_ezlcard,
...
WITH base AS (
    ...
    FROM public.teffdata e
    WHERE {BASE_FILTER}
      AND NULLIF(BTRIM(e."FACTORY"::text), '') IS NOT NULL
),
factory_total AS (
    SELECT factory,
...
SELECT factory, eff_pct
FROM factory_total
ORDER BY eff_pct DESC NULLS LAST, factory
""")
...
latest_data AS (
    ...
    INNER JOIN latest_by_factory l
      ON p.factory = l.factory AND p.produce_date = l.latest_date
...
ORDER BY
    CASE l.factory WHEN 'G1' THEN 1 WHEN 'G2' THEN 2 WHEN 'G3' THEN 3 WHEN 'G4' THEN 4 WHEN 'TRM' THEN 5 WHEN 'EA' THEN 6 ELSE 99 END,
...
ORDER BY DATE_TRUNC('month', e."Date"::date),
         CASE UPPER(BTRIM(e."FACTORY"::text)) WHEN 'G1' THEN 1 WHEN 'G2' THEN 2 WHEN 'G3' THEN 3 WHEN 'G4' THEN 4 WHEN 'TRM' THEN 5 WHEN 'EA' THEN 6 ELSE 99 END
""")
...
FROM public.teffdata e
WHERE {BASE_FILTER}
  AND e."Date"::date IN (SELECT d FROM filtered_dates)
  AND NULLIF(BTRIM(e."FACTORY"::text), '') IS NOT NULL
...
FILTERS_SQL = text("""
SELECT MIN(e."Date"::date) AS min_date,
...
WHERE table_schema = 'public' AND table_name = 'teffdata'
